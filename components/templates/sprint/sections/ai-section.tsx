      ...
      <div className="w-full flex justify-center px-6">
        <div className="w-full max-w-5xl">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }}
            ...
            <span className="text-white font-medium">Sprint for Agents.</span> Choose from a variety of AI agents and start delegating work, from code generation to other technical tasks.
          </motion.p>
                    ...
                    <div key={agent.name} style={agent.selected ? { transform: "scale(1.04) rotateX(17deg)", background: "linear-gradient(#343434 0%, #2d2d2d 100%)", borderRadius: "6px", height: "48px", position: "relative", boxShadow: "inset 0 -2.75px 4.75px rgba(255, 255, 255, 0.14), inset 0 -0.752px 0.752px rgba(255, 255, 255, 0.1), 0 54px 73px 3px rgba(0, 0, 0, 0.5)", zIndex: 20, marginLeft: "-12px", marginRight: "-12px" } : { opacity: 1 - index * 0.15, height: "42px" }}>
                      <div className="flex items-center justify-between h-full" style={{ paddingLeft: "24px", paddingRight: "24px", gap: "12px" }}>
                        <div className="flex items-center gap-3">
                          ...
                          <span className={agent.selected ? "text-white font-medium" : "text-zinc-300"}>{agent.name}</span>
                          {agent.isAgent && <span className="text-xs bg-zinc-700 text-zinc-400 px-2 py-0.5 rounded">Agent</span>}
                        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.5 }} className="mt-16">
            <div className="grid grid-cols-1 md:grid-cols-2">
              <div className="border-t border-r border-b border-zinc-800/60 pt-12 pr-12 pb-16">
                ...
                <p className="text-zinc-500 text-base mb-8">Streamline your product development workflows with AI assistance for routine, manual tasks.</p>
                <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-xl p-5">
                  <div className="flex items-center gap-2 mb-5">
                    ...
                    <span className="text-zinc-500 text-sm">Triage <span className="text-zinc-300">Intelligence</span></span>
                  </div>
                  <div className="flex items-center gap-3 mb-3">
                    <span className="text-zinc-600 text-sm w-20">Suggestions</span>
                    <div className="flex items-center gap-2">
                      <span className="flex items-center gap-1.5 rounded-md px-2 py-1 text-sm" style={{ background: "#7170ff" }}>
                        <span className="w-4 h-4 bg-white/30 rounded-full" /><span className="text-white">nan</span>
                      </span>
                      <span className="flex items-center gap-1.5 bg-zinc-800/30 rounded-md px-2 py-1 text-sm text-zinc-600">
                        <span className="w-3 h-3 border border-zinc-700 rounded" />Mobile App Refactor
                      </span>
                      <span className="flex items-center gap-1.5 text-sm text-zinc-700">
                        <span className="w-2 h-2 bg-zinc-600 rounded-full" />Slack
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 mb-3"><span className="text-zinc-600 text-sm w-20">Duplicate of</span></div>
                  <div className="flex items-center gap-3 mb-4"><span className="text-zinc-600 text-sm w-20">Related to</span></div>
                  <div className="bg-zinc-800/40 rounded-lg p-4 ml-4">
                    <div className="flex items-center gap-2 mb-3"><span className="w-5 h-5 bg-zinc-600 rounded-full" /><span className="text-zinc-300 text-sm font-medium">nan</span></div>
                    <p className="text-zinc-500 text-xs mb-2">Why this assignee was suggested</p>
                    <p className="text-zinc-500 text-sm mb-4">This person was the assignee on previous issues related to performance problems in the mobile app launch flow</p>
                    <p className="text-zinc-500 text-xs mb-2">Alternatives</p>
                    <div className="flex items-center gap-2 mb-4">
                      <span className="flex items-center gap-1.5 bg-zinc-700/50 rounded-md px-2 py-1 text-sm"><span className="w-4 h-4 bg-zinc-500 rounded-full" /><span className="text-zinc-400">yann</span></span>
                      <span className="flex items-center gap-1.5 bg-zinc-700/50 rounded-md px-2 py-1 text-sm"><span className="w-4 h-4 bg-zinc-500 rounded-full" /><span className="text-zinc-400">erin</span></span>
                    </div>
              ...
              <div className="border-t border-b border-zinc-800/60 pt-12 pl-12 pb-16">
                ...
                <p className="text-zinc-500 text-base mb-8">Connect Sprint to your favorite tools including Cursor, Claude, ChatGPT, and more.</p>
                <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-xl p-5 font-mono text-sm">
                  ...
                  <div className="space-y-1 mb-6">
                    <p><span className="text-orange-400/70">&quot;mcpServers&quot;</span><span className="text-zinc-500">: {"{"}</span></p>
                    <p className="pl-4"><span className="text-orange-400/70">&quot;sprint&quot;</span><span className="text-zinc-500">: {"{"}</span></p>
                    <p className="pl-8"><span className="text-orange-400/70">&quot;command&quot;</span><span className="text-zinc-500">: </span><span className="text-green-400/70">&quot;npx&quot;</span></p>
                  </div>
                  <div className="bg-zinc-800/40 rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-4"><span className="w-0.5 h-5 bg-zinc-600" /><span className="text-zinc-600">Ask anything</span></div>
                    <div className="flex items-center gap-2">
                      <button className="flex items-center gap-1.5 border border-zinc-700/60 text-zinc-500 text-sm px-3 py-1.5 rounded-full hover:bg-zinc-700/30 transition-colors"><Paperclip className="w-3.5 h-3.5" /> Attach</button>
                      <button className="flex items-center gap-1.5 border border-zinc-700/60 text-zinc-500 text-sm px-3 py-1.5 rounded-full hover:bg-zinc-700/30 transition-colors"><Globe className="w-3.5 h-3.5" /> Search</button>
                      <button className="flex items-center gap-1.5 border border-zinc-700/60 text-zinc-500 text-sm px-3 py-1.5 rounded-full hover:bg-zinc-700/30 transition-colors"><Lightbulb className="w-3.5 h-3.5" /> Reason</button>
                    </div>
                  </div>
                </div>
              </div>
